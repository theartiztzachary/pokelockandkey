...
const struct TrainerMon *CombinePools(u16 trainerId)
{
    ...
    poolparty = (TrainerMon*)malloc(sizeof(TrainerMon) * classPoolSize * routePoolSize);

    switch(currentTrainerClass)
    {
        case 0: //hiker
            memcpy(poolparty, ClassParty_Hiker, classPoolSize * sizeof(TrainerMon));
            break;
        case 1: //team aqua (grunt)
            memcpy(poolparty, ClassParty_Team_Aqua, classPoolSize * sizeof(TrainerMon));
            break;
        case 2: //pkmn breeder
            memcpy(poolparty, ClassParty_Pkmn_Breeder, classPoolSize * sizeof(TrainerMon));
            break;
        case 3: //cool trainer
            memcpy(poolparty, ClassParty_Cooltrainer, classPoolSize * sizeof(TrainerMon));
            break;
        default:
            memcpy(poolparty, ClassParty_Hiker, classPoolSize * sizeof(TrainerMon));
            break;
